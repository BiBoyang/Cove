...
- [ ] vault 下载 8MB 批量写盘占主线程（审查登记 2026-09-15，中，1.x）：@MainActor + 同步 FileHandle 写，GB 级仓库=数百次主线程写盘；挪后台队列 + 批量落盘
- [ ] 首页封面 stat 挤主车道（审查登记 2026-09-15，小-中）：封面存在性 stat 与 mpv 读串行竞争主 SMB 车道，起播阶段可能卡顿；挪预热车道或本地标记
- [ ] 缓存驱逐节奏盲区（审查登记 2026-09-15，小）：软预算 + 最终一致，关预热/纯 vault 会话才显形；补周期性对账或事件触发驱逐

...
状态：**闭卡 2026-09-14（环境漂移）**。路线 D 真机复测通过；副产物发现
「控制条遮挡底部字幕」真缺陷，另立候选登记。
...
真机复测：本地仓库 → 字幕夹具 → E → GbkMovie.mkv，控制条自动隐藏后
两个字幕窗口均正确显示（「外挂字幕第一行：你好，世界」「第二行：
Cove 验收样张」）。三重排除齐备：解码链 spike（12 组实验）/ 原文件
字节级验尸 + 直喂 spike / 真机渲染。按环境漂移闭卡——09-12 夜二进制
现场不可复得（当夜工作树构建，次日森林重建覆盖）。若复发：spike
（build/spike-subtitle/，未跟踪区）+ `sub-text` 观察路径已实证可用，
直接重开本卡。副产物：控制条可见时遮挡底部字幕（真 UX 缺陷，用户
复测时亲历），登记 BACKLOG 候选另立卡。

...
final class SettingsPaneViewController: NSViewController {
    ...
    override func loadView() {
        ...
        // The document view tracks the viewport width (vertical scrolling
        // only) so the form column below can cap itself and stay leading.
        let content = NSView()
    ...
    /// The pane reloads every time it becomes the visible destination:
    /// settings may have changed elsewhere, and the usage numbers move
    /// constantly. (The old window did the same in `showWindow`.)
    ...
    /// persists it as a security-scoped bookmark. It sheets on the main
    /// window — the pane's host — instead of the retired settings window.
...
extension SettingsPaneViewController: NSTextFieldDelegate {
...
extension SettingsPaneViewController: NSTableViewDataSource, NSTableViewDelegate {
